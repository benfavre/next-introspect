        ...
        try {
          return {
            hasMiddleware: await this.fileExists(
              path2.join(projectPath, "middleware.js")
            ) || await this.fileExists(
              path2.join(projectPath, "middleware.ts")
            ) || await this.fileExists(
              path2.join(projectPath, "src/middleware.js")
            ) || await this.fileExists(
              path2.join(projectPath, "src/middleware.ts")
            )
          };
        } catch (error) {
          const errorMessage = error instanceof Error ? error.message : String(error);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (this.isAppRouterSpecialFile(entry.name)) {
          const relativeToApp = path2.relative(appDir, entry.path);
    ...
    try {
      ...
      for (const entry of entries) {
        ...
        if (this.isPagesRouterFile(entry.name)) {
          const relativeToPages = path2.relative(pagesDir, entry.path);
    ...
    function traverse(obj, currentPath = []) {
      for (const [key, value] of Object.entries(obj)) {
        ...
        if (value && typeof value === "object" && !Array.isArray(value)) {
          if (value.router) {
            const path4 = newPath.filter((segment) => segment !== "").join("/");
            result.push({
              ...value,
              path: path4 ? `/${path4}` : "/"
            });
...
var NextIntrospect = class {
  ...
  getResult() {
    ...
    const routes = this.options.outputFormat?.nested ? routesToNested(this.routes, this.options.outputFormat.includeEmptySegments) : this.routes;
    return {
