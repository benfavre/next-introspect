...
var TypeScriptFormatter = class {
  ...
  generateRouteObject(routeStructure) {
    ...
    let output = "// Named exports for tree-shaking\n";
    const topLevelKeys = Object.keys(routeStructure);
    for (const key of topLevelKeys) {
      output += `export const ${key} = `;
      output += this.generateObjectRecursive(routeStructure[key], 1);
      output += ";\n\n";
    }
    output += "// Getter-based routes object for dot notation (tree-shakable)\n";
    output += "export const routes = {\n";
    for (const key of topLevelKeys) {
      output += `${indentStr}get ${key}() {
`;
      output += `${indentStr}${indentStr}return ${key};
`;
      output += `${indentStr}},
`;
    }
