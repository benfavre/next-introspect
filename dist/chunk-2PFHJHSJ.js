...
var TypeScriptFormatter = class {
  ...
  generateHeader(result) {
    const lines = [
      "/**",
      ` * Generated route types for ${result.project.name || "Next.js project"}`,
      ` * Generated on: ${result.metadata.analyzedAt.toISOString()}`,
      ` * Mode: ${result.metadata.mode}`,
      ` * Routes: ${Array.isArray(result.routes) ? result.routes.length : "nested"}`,
      " *",
