...
var TypeScriptFormatter = class {
  ...
  generateRouteObject(routeStructure) {
    ...
    for (const exp of allExports) {
      ...
      output += this.generateValueRecursive(exp.value, exp.path);
      output += ";\n";
    }
   ...
   */
  generateValueRecursive(value, currentPath = []) {
    if (typeof value === "string") {
      if (value.includes("[") && value.includes("]")) {
        ...
        const routeKey = currentPath.join("_");
        const routeMetadata = this.metadata?.[routeKey] || this.metadata?.[strippedPath];
        let result = "(() => {\n";
        result += `  /**
`;
...
`;
        if (routeMetadata?.title) {
          result += `  func.title = "${routeMetadata.title}";
`;
        }
        if (routeMetadata?.description) {
          result += `  func.description = "${routeMetadata.description}";
`;
        }
        result += `  return func;
`;
        ...
        const path3 = this.stripPathPrefixes(value);
        const routeKey = Array.isArray(currentPath) ? currentPath.join("_") : currentPath;
        const routeMetadata = this.metadata?.[routeKey] || this.metadata?.[path3];
        let result = `{ path: "${path3}"`;
        if (routeMetadata?.title) {
          result += `, title: "${routeMetadata.title}"`;
        }
        if (routeMetadata?.description) {
          result += `, description: "${routeMetadata.description}"`;
        }
        result += ` }`;
        return result;
      }
...
var NextIntrospect = class {
  ...
  format(format, metadata) {
    ...
    if (format === "typescript") {
      const options = {};
      if (this.options.outputFormat?.stripPrefixes) {
        options.stripPrefixes = this.options.outputFormat.stripPrefixes;
      }
      if (metadata) {
        options.metadata = metadata;
      }
      if (Object.keys(options).length > 0) {
        formatter = new TypeScriptFormatter(options);
      }
    }
  ...
  async exportToFile(filePath, format = "json", options) {
    const fs = await import("fs/promises");
    const path3 = await import("path");
    const result = this.format(format, options?.metadata);
