...
var TypeScriptFormatter = class {
  ...
  stripPathPrefixes(path3) {
    ...
    console.log(`Stripping prefixes from "${path3}":`, this.stripPrefixes);
    for (const prefix of this.stripPrefixes) {
      ...
      if (prefix.startsWith("//") && prefix.endsWith("//") && prefix.length > 4) {
        ...
        if (match) {
          ...
          const result = stripped.startsWith("/") ? stripped : `/${stripped}`;
          console.log(`    Returning (regex): "${result}"`);
          return result;
        }
      } else {
        console.log(`    String prefix check: "${path3}".startsWith("${prefix}") = ${path3.startsWith(prefix)}`);
