...
var BaseAdapter = class {
  ...
  async validateProjectPath(projectPath) {
    ...
    try {
      ...
      throw new Error(`Cannot access project path ${projectPath}: ${error.message}`);
    }
  }
  /**
...
var NextJsAdapter = class extends BaseAdapter {
    ...
    for (const configFile of configFiles) {
      ...
      if (await this.fileExists(configPath)) {
        try {
          ...
          console.warn(`Warning: Could not parse Next.js config ${configFile}:`, error.message);
        }
      }
    }
    return void 0;
  ...
  async getAppRouterRoutes(projectPath, mode) {
    ...
    try {
      ...
      console.warn("Warning: Could not analyze App Router routes:", error.message);
    }
    return routes;
  }
  /**
  ...
  async getPagesRouterRoutes(projectPath, mode) {
    ...
    try {
      ...
      console.warn("Warning: Could not analyze Pages Router routes:", error.message);
    }
    return routes;
  }
  /**
