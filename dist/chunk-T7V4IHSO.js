...
`;
    ...
    if (typeof value === "object" && value !== null) {
      const hasOnlyRouteProps = Object.keys(value).every(
        (k) => ["index", "base", "get"].includes(k)
      );
      if ((value.index || value.base) && typeof (value.index || value.base) === "string" && hasOnlyRouteProps) {
...
`;
          result2 += `  func.index = "${strippedPath}";
`;
          result2 += `  return func as typeof func & { index: string };
`;
          result2 += "})()";
          return result2;
        } else {
  ...
  generateGetterObjectRecursive(obj, currentPath) {
    ...
    for (const [key, value] of sortedEntries) {
      ...
      const isSimpleRoute = typeof value === "string" || typeof value === "object" && value && (value.index || value.base) && Object.keys(value).every(
        (k) => ["index", "base", "get"].includes(k)
      );
      if (isSimpleRoute) {
