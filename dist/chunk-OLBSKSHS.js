...
var TypeScriptFormatter = class {
  ...
  generateRouteObject(routeStructure) {
    ...
    output += "\n// Getter-based routes object for dot notation (tree-shakable)\n";
  ...
  generateValueRecursive(value) {
    if (typeof value === "string") {
      if (value.includes("[") && value.includes("]")) {
        ...
        const templateType = this.buildTemplateType(strippedPath, params);
        let result = "Object.assign(\n";
        result += `  (({ ${paramTypes} }: { ${paramTypes} }): ${templateType}) => \`${this.buildTemplateString(strippedPath, params)}\`,
`;
    ...
    if (typeof value === "object" && value !== null) {
      if (value.path && typeof value.path === "string") {
        ...
        if (value.get) {
          ...
          result2 += `  (({ ${paramObj} }: { ${paramObj} }): ${templateType}) => \`${this.buildTemplateString(strippedPath, params)}\`,
`;
          result2 += `  { path: "${strippedPath}" }
`;
          result2 += ")";
          return result2;
        } else {
   ...
   */
  buildTemplateType(path3, params) {
    let result = "`";
    let paramIndex = 0;
  ...
  /**
   * Generate getter-based object that mirrors the structure
   */
...
`;
        ...
        result += `  get ${key}() {
`;
        result += `    return ${this.generateGetterObjectRecursive(value, newPath)};
`;
        result += `  },
`;
      }
...
`;
          result += `${currentIndent}${indentStr}get: ({ ${paramTypes} }: { ${paramTypes} }) => \`${this.buildTemplateString(strippedPath, params)}\`,
`;
