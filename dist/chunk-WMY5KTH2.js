...
`;
        result += `  get: ({ ${paramTypes} }: { ${paramTypes} }) => \`${this.buildTemplateString(strippedPath, params)}\`
`;
        result += "}";
        return result;
      } else {
    ...
    if (typeof value === "object" && value !== null) {
      if (value.path && typeof value.path === "string") {
        let result2 = "{\n";
        result2 += `  path: "${this.stripPathPrefixes(value.path)}"`;
        if (value.get) {
          const getFunction = value.get.replace(/`[^`]*`/g, (match) => {
            const templateContent = match.slice(1, -1);
            const strippedContent = this.stripPathPrefixes(templateContent);
