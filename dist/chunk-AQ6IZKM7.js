...
var TypeScriptFormatter = class {
  ...
  stripPathPrefixes(path3) {
    ...
    for (const prefix of this.stripPrefixes) {
      if (prefix.startsWith("//") && prefix.endsWith("//") && prefix.length > 4) {
        ...
        console.log(`Testing regex pattern "${regexPattern}" on path "${path3}"`);
        const regex = new RegExp(`^${regexPattern}`);
        const match = path3.match(regex);
        if (match) {
          console.log(`Regex match found: "${match[0]}"`);
          return path3.slice(match[0].length) || "/";
        } else {
          console.log(`No regex match`);
        }
      } else {
        if (path3.startsWith(prefix)) {
          console.log(`String prefix match: "${prefix}"`);
          return path3.slice(prefix.length) || "/";
        }
      }
    }
    return path3;
  ...
  buildRouteStructure(routes) {
    ...
    for (const route of sortedRoutes) {
      ...
      const strippedPath = this.stripPathPrefixes(route.path);
      const pathParts = this.parseRoutePath(strippedPath);
      ...
      for (let i = 0; i < pathParts.length; i++) {
        ...
        if (isLast) {
          if (current[part] && typeof current[part] === "object") {
            current[part].index = strippedPath;
  ...
  generateRouteObject(routeStructure) {
    ...
    let output = "export const routes = ";
    const generateObjectRecursive = (obj, depth = 1) => {
      ...
      for (const [key, value] of entries) {
        ...
        if (key === "index" && typeof value === "string") {
          result += `${currentIndent}path: "${value}",
`;
          continue;
        }
...
`;
            result += `${currentIndent}${indentStr}path: "${value}",
`;
            result += `${currentIndent}${indentStr}get: ({ ${paramTypes} }: { ${paramTypes} }) => \`${this.buildTemplateString(value, params)}\`,
`;
...
`;
          ...
          if (value.index && Object.keys(value).length === 1) {
            result += `${currentIndent}${key}: {
`;
            result += `${currentIndent}${indentStr}path: "${value.index}",
`;
            result += `${currentIndent}},
`;
          } else {
    ...
    output += " as const;\n\n";
    output += "export type Routes = typeof routes;\n";
    return output;
  }
  /**
