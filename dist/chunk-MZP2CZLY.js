...
var NextJsAdapter = class extends BaseAdapter {
  ...
  async getProjectInfo(projectPath, packageDisplayOptions) {
    ...
    const packageInfo = await getPackageInfo(projectPath, packageDisplayOptions);
  ...
  convertFilePathToRoute(relativePath) {
    let routePath = relativePath.replace(/\/(page|route|layout|loading|error|not-found|template|default)(\..*)?$/, "");
...
var NextIntrospect = class {
  ...
  async analyze() {
    ...
    this.projectInfo = await adapter.getProjectInfo(this.projectPath, this.options.packageDisplay);
